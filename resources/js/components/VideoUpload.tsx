...
export const VideoUpload = ({ gameId, onUploadComplete, onCancel }: VideoUploadProps) => {
    ...
    if (uploadMutation.isSuccess && !isProcessing) {
        ...
        return (
            <div className="flex flex-col items-center space-y-4 rounded-lg bg-green-50 p-6">
    ...
    if (uploadMutation.isPending || isProcessing) {
        return (
            <div className="flex flex-col items-center space-y-4 rounded-lg bg-blue-50 p-6">
                <div className="flex h-12 w-12 animate-spin items-center justify-center rounded-full bg-blue-600">
                    <svg className="h-6 w-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path
                            strokeLinecap="round"
                ...
                {isProcessing && (
                    <p className="text-center text-sm text-blue-600">
                        We're compressing your video to ensure optimal quality and file size. This may take a few moments...
                    </p>
                )}
            </div>
        );
    ...
    if (uploadMutation.isError) {
        return (
            <div className="flex flex-col items-center space-y-4 rounded-lg bg-red-50 p-6">
                ...
                <p className="text-center text-red-700">{uploadMutation.error?.message}</p>
                <div className="flex space-x-2">
                    <Button onClick={() => uploadMutation.reset()} variant="outline">
                        Try Again
                    </Button>
                    {onCancel && (
    ...
    return (
        <div className="space-y-6">
            <div className="text-center">
                <h2 className="mb-2 text-2xl font-bold text-gray-900">Record Game Video</h2>
                <p className="text-gray-600">Record your Spikeball game to capture the highlights and review plays later.</p>
            </div>

            <VideoRecorder onVideoReady={handleVideoReady} onCancel={onCancel} />

            <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4">
                <h4 className="mb-2 font-semibold text-yellow-800">📹 Video Guidelines</h4>
